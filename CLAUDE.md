...
- 🔄 **Input bindings are actions now, stored by key POSITION (WP-6.4 part 1, 2026-08-14)**: `builtin.controllers.bindings` holds the layer — `Control` (a physical control with a canonical string form like `Key:W` / `GamepadButton:DPadUp`), `BindingTable` (live, lock-guarded, JSON round-trip, capture mode for rebinding), and `IInputModifier` (`deadzone`, `curve`, `invert`, `range`). Shipped button bindings live in **`models/nogame.bindings.json`**, declared as a resource; user rebindings persist to `bindings.json` under `Engine.RWPath` and are layered per-action over the defaults, so bindings added by an update still appear. `InputMapper` consults the new table first and falls through to the legacy flat `MapButtonToLogical` (retained as public API, but **empty in the shipped config — the entries moved, they were not duplicated**, or a rebind would leave the old key firing too). **Store `ScanCode`, never the key string**: `"w"` is a position that merely looks like a character (on AZERTY that key prints Z), and a rebinding UI must ask the platform for the display label separately. **Part 2 is still open**: `InputController` still reads raw codes for WASD/triggers/stick, and its `StickTransfer` x⁴ curve and trigger range-remap are the `curve 4` and `range -1 1 0 255` modifiers waiting to replace them.
