...
- ✅ TALE-SOCIAL Phase D4 (Seedability validation): wired up `tests/JoyceCode.Tests/JoyceCode.Tests.csproj` (xUnit, references Joyce.csproj). Added 29 unit tests for the bake module (`ScenarioFileName`, `ScenarioExporter`, `ScenarioApplicator`, `ScenarioSelector`, `ScenarioCompiler`) plus revived 10 pre-existing `RoutingPreferencesTests`. All 39 tests pass in 170 ms — `dotnet test tests/JoyceCode.Tests/JoyceCode.Tests.csproj`. The compiler determinism test runs `CompileInMemory` twice with the same seed (12 NPCs, 5 simulation days) and asserts byte-for-byte equality — that's the core seedability assertion the D4 plan calls for. Pre-existing test files under `engine/navigation/` and `engine/streets/` are excluded from compilation pending API updates / DI fixture work; csproj has an explanatory comment. See `docs/roadmap/proposed/TALE-SOCIAL-PHASES.md`.
