...
public sealed class EnginePreviewService
{
    ...
    public void Initialize(GlInterface glInterface)
    {
        ...
        // The renderer owns this: we hand over a GetProcAddress delegate and never name a
        // graphics-API type ourselves (WP-0.2).
        if (string.IsNullOrEmpty(engine.GlobalSettings.Get("platform.threeD.API")))
        {
            PreviewHelper.DetectAndSetGlVersion(glInterface.GetProcAddress);
        }
