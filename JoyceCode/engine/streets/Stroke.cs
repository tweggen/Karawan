...
public class StrokeConverter : JsonConverter<Stroke>
{
        ...
        JsonSerializerOptions options)
    {
        ...
        while (reader.Read())
        {
            if (reader.TokenType == JsonTokenType.EndObject)
            {
                ...
                var stroke = clusterDesc.StrokeStore().GetStroke(sid);
                return stroke;
            }
            ...
            if (reader.TokenType == JsonTokenType.PropertyName)
            {
                ...
                switch (propertyName)
                {
                    case nameof(Stroke.Sid):
                        sid = reader.GetInt32();
                        break;
                    case nameof(Stroke.ClusterId):
        ...
        JsonSerializerOptions options)
    {
        ...
        writer.WriteNumber(nameof(Stroke.Sid), stroke.Sid);
        writer.WriteNumber(nameof(Stroke.ClusterId), stroke.ClusterId);
    ...
    public float GetAngleSP(in StreetPoint sp)
    {
        ...
        else
        {
            throw new InvalidOperationException(
                $"Stroke.getAngleSP(): Invalid stroke #{Sid} that does not contain street point {sp.Id}");
        }
