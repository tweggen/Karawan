...
| **WP-6.3** | ⏳ **step 1 MERGED, steps 2–4 PR-OPEN** | `platform/wp-6.3-scancodes` → `platform/wp-6.3-device-contracts` | [#73](https://github.com/tweggen/Karawan/pull/73) merged; [#74](https://github.com/tweggen/Karawan/pull/74) open | 1 | build ✅ · **234/234 tests** ✅ · ScanCode ≡ SDL_Scancode 104/104 ✅ | **GATE-C input 🟡 macOS: WASD ✅, text entry ✅** | **Native input semantics.** Step 1: `ScanCode` on USB HID usage IDs, so `Sdl3KeyCodes` is a **cast**, not a table; the string table moved to `engine.inputs.ScanCodeNames` so there is one, not one per backend — **confirmed on real hardware, WASD works**. Steps 2–4: devices carry no events, `OnConnectionChanged` replaced by `INPUT_DEVICE_ATTACHED/DETACHED` on the queue (**a race fix, not tidiness**), enumeration is an immutable `IReadOnlyList` snapshot. **Found and fixed a pre-existing desktop regression while validating — see below.** Not yet confirmed: F8, arrows, Android re-check. |
