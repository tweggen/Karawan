...
**Last updated:** 2026-08-14 (Phase 4 complete — Assimp is out of the shipped game)
...
### ✅ 2026-08-14 — PHASE 4 COMPLETE: FBX import is a build step, Assimp is out of the game

WP-4.1 through 4.4 on `platform/wp-4`, one commit each. Models are baked into
`mo-{hash}` files by Chushi and read back at runtime with no importer present.
**The APK contains no `libassimp.so`** — 16 native libraries, 0 fbx assets, 13
`mo-` and 14 `ac-`. The asset count moving 167 → 155 is exactly −25 fbx +13 bakes.
...
| GATE-D | Animation correct on macOS + Windows | 🟡 **PASSED 2026-08-06 for Phase 1** — Windows confirmed, macOS confirmed on a **Debug** build (Release does not currently start, see known issue KI-1). **RE-OPENED for Phase 4 sign-off, 2026-08-14:** models now come from baked `mo-{hash}` files rather than from Assimp, and the one thing no automated check covers is whether walk / idle / death still *look* right. AC-4.2 compares the two load paths field by field (13/13, bone order exact), so the risk is low — but bone-order faults render a plausible wrong pose rather than failing, which is exactly why this gate exists. |
