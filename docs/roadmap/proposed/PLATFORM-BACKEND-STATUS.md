...
it. **KI-11 therefore BLOCKED the rest of this criterion** — which is why WP-5.3 was pulled forward
ahead of WP-6.4.

**KI-11 is fixed as of 2026-08-11 (WP-5.3, #76), so the hit-test is now ANSWERABLE and still
UNANSWERED.** The owner reports the desktop UI "works better than it ever did", which establishes
that it renders and takes input — but it does not by itself establish the HiDPI half, because that
needs clicks observed landing at the cursor **on a scaled display specifically**. A transposed
logical/pixel pair renders perfectly and misses by the scale factor, so a build that looks right on
a 1× monitor says nothing. One deliberate check on the retina Mac closes this. **Still unrun: HiDPI hit-testing (blocked), and Linux entirely.** Resize confirmed 2026-08-10 on the post-WP-3.5 build, i.e. against SDL3 with no Silk fallback present. |
