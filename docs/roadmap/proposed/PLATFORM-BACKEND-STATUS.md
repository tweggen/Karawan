...
| **KI-19** | ✅ **FIXED 2026-08-23. Mouse-look was capped at the window borders.** Turning the camera with the mouse stopped as soon as the (hidden) pointer reached an edge of the window, which reads as a deliberate rotation limit rather than as broken input. Cause: `InputController._desktopMouseController` derived the look delta by DIFFERENCING two absolute pointer positions, and `Sdl3WindowBackend` forwarded only `ev.motion.x/y`. That worked under Silk, where `CursorMode.Raw` reports an UNBOUNDED position; SDL3 reports the position clamped to the window and, in relative-mouse mode (which is what `SetMouseVisible(false)` turns on), warped as well — so the difference goes to zero at the border while `xrel`/`yrel`, the field that carries the real motion, was being dropped on the floor. **Entered at WP-3.2** with the rest of the desktop input rewrite, alongside KI-14 and #61, and shares their shape: nothing failed, no log line appeared, the input just quietly became wrong. Fix: `IWindowBackend.OnMouseMoved` now carries `(position, delta)`, the delta travels on the new `engine.news.Event.PhysicalDelta`, and `InputController` accumulates it — the absolute position stays for hit-testing and the touch paths, which legitimately need it. `_desktopMouseController` also DROPS accumulated motion while a mouse button is held rather than banking it, or releasing the button would snap the camera by the whole drag. | fixed. **Found by the owner playing the game**, which is the third desktop-input regression from that phase found that way and not by a check. Worth noting `debug.option.mouseEnabled=true` (and any DEBUG Linux run, which forces it) leaves relative mode OFF, so the OS pointer itself stops at the screen edge — the deltas are then honest but still bounded. That is a separate condition, not this bug. |
